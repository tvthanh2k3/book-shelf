...
def _row_to_dict(review, book_title, author_name):
    return {
        "id": review.id,
...
async def get_all(db: AsyncSession, skip: int, limit: int):
    stmt = _detail_select().order_by(Review.id.desc()).offset(skip).limit(limit)
...
async def _get_by_id_with_details(db: AsyncSession, review_id: int):
    stmt = _detail_select().where(Review.id == review_id)
    result = await db.execute(stmt)
...
async def create(db: AsyncSession, data: ReviewCreate):
    ...
    return await _get_by_id_with_details(db, review.id)
...
async def update(db: AsyncSession, review_id: int, data: ReviewUpdate):
    ...
    await db.commit()
    return await _get_by_id_with_details(db, review_id)
